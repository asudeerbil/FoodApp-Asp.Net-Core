...
(function (global, factory) {
  ...
  function report (_ref) {
    ...
    if (conflictsCount > 0) {
      console.primary("%cConflict".concat(conflictsCount > 1 ? 's' : '', " found:"), 'color: darkred; font-size: large');
      var data = {};
    ...
    if (noConflictsCount > 0) {
      console.primary("%cNo conflict".concat(noConflictsCount > 1 ? 's' : '', " found with ").concat(noConflictsCount == 1 ? 'this' : 'these', ":"), 'color: green; font-size: large');
      var _data = {};
    ...
    if (timeOutCount > 0) {
      console.primary("%cLeftovers--we timed out before collecting test results for ".concat(timeOutCount == 1 ? 'this' : 'these', ":"), 'color: blue; font-size: large');
      var _data2 = {};
    ...
    return new Promise(function (resolve, reject) {
      ...
      function poll(duration, cumulativeDuration) {
        setTimeout(function () {
          ...
          if (showProgress) {
            console.primary(progressIndicator);
          }
  ...
  function conflictDetection() {
    ...
    if (testCount === 0) {
      console.primary('%cAll Good!', 'color: green; font-size: large');
      console.primary('We didn\'t find anything that needs testing for conflicts. Ergo, no conflicts.');
    } else {
      console.primary("Testing ".concat(testCount, " possible conflicts."));
      console.primary("We'll wait about ".concat(Math.round(WINDOW.FontAwesomeDetection.timeout / 10) / 100, " seconds while testing these and\n") + "then up to another ".concat(Math.round(WINDOW.FontAwesomeDetection.resultsCollectionMaxWait / 10) / 100, " to allow the browser time\n") + "to accumulate the results. But we'll probably be outta here way before then.\n\n");
      console.primary("You can adjust those durations by assigning values to these attributes on the <script> element that loads this detection:");
      console.primary("\t%c".concat(timeoutAttr, "%c: milliseconds to wait for each test before deciding whether it's a conflict."), 'font-weight: bold;', 'font-size: normal;');
      console.primary("\t%c".concat(resultsCollectionMaxWaitAttr, "%c: milliseconds to wait for the browser to accumulate test results before giving up."), 'font-weight: bold;', 'font-size: normal;');
      pollUntil({
        ...
        if (e === 'timeout') {
          console.primary('TIME OUT! We waited until we got tired. Here\'s what we found:');
          setDoneResults({
            nodesTested: nodesTested,
            nodesFound: nodesFound
          });
          ...
          console.primary('Whoops! We hit an error:', e);
          console.primary('Here\'s what we\'d found up until that error:');
          setDoneResults({
            nodesTested: nodesTested,
            nodesFound: nodesFound
          });
